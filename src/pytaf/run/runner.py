...
def _is_windows_reparse_point(path: Path) -> bool:
    ...
    try:
        out = subprocess.run(
            ["cmd", "/c", "fsutil", "reparsepoint", "query", str(path)], capture_output=True, text=True
        )
    ...
    if IS_WIN:
        ...
        if tgt.exists() and _is_windows_reparse_point(tgt):
            with suppress(Exception):
                subprocess.run(
                    ["cmd", "/c", "rmdir", str(tgt)], check=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL
                )
        if not tgt.exists():
            try:
                subprocess.run(
...
def _spawn_broker(env: dict[str, str]) -> tuple[subprocess.Popen[bytes], object | None, bool]:
    ...
    attached = False
    if IS_WIN:
        from pytaf.broker.jobobjects_win import CREATE_NEW_PROCESS_GROUP, assign_process, create_kill_on_close_job
        ...
        try:
            ...
            if hasattr(proc, "_handle"):
                assign_process(job, int(proc._handle))
                attached = True
        except Exception:
            attached = False
...
def run(resolved_results_root: Path, bench_cfg: dict[str, Any], relocation_meta: dict[str, Any]) -> int:
    ...
    try:
        ...
        headers.write_headers(
            run_dir, bench_cfg, ipc_version=ipc_version, epoch_id=epoch_id, ipc_handshake_ok=ipc_handshake_ok
        )
        ...
        write_pidfile(run_uuid=run_dir.name, pid=proc.pid, dir_=run_dir)
        ...
        try:
            ipc.write_msg(
                w_parent,
                {
                    "op": "xact", "epoch_id": epoch_id, "op_id": 3,
                    "resource": uri, "lane": "bulk", "payload": bulk_payload
                },
            )
        except ValueError:
        ...
        ipc.write_msg(
            w_parent, {
                "op": "xact", "epoch_id": epoch_id, "op_id": 4, 
                "resource": uri, "lane": "control", "payload": b"*IDN?"
            }
        )
        ipc.write_msg(
            w_parent, {
                "op": "xact", "epoch_id": epoch_id, "op_id": 7, 
                "resource": uri, "lane": "control", "payload": b"*IDN?"
            }
        )
        ...
        first_control_payload_any = (ack_a if ack_ids[0] in (4, 7) else (ack_b if ack_ids[1] in (4, 7) else ack_c)).get(
            "payload"
        ) or b"PYTAF,ECHO,1.0\n"
        first_control_payload = (
            first_control_payload_any
            if isinstance(first_control_payload_any, bytes)
            else b"PYTAF,ECHO,1.0\n"
        )
...
def _ladder_terminate(proc: subprocess.Popen[bytes], job: Any) -> tuple[int, str]:
    """
...
def _terminate_broker(proc: subprocess.Popen[bytes], job: Any) -> None:
    try:
        proc.terminate()
        proc.wait(timeout=2)
    except Exception:
